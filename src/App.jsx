...
function Logo({ small }) {
  ...
  return (
    <img
      src="/favicon_atelierlogistique.png"
      alt="Atelier Logistique"
      ...
      <nav className="border-b border-slate-200 bg-white sticky top-0 z-20">
        <div className="max-w-[1600px] mx-auto px-6 py-4 flex items-center justify-between flex-wrap gap-3">
          <div className="flex items-center gap-5">
            ...
            <div className="w-px h-5 bg-slate-200" />
            <a href="/outils/incoterms-2020" target="_blank" rel="noopener" className="inline-flex items-center gap-1.5 px-2.5 py-1.5 rounded-md font-jetbrains text-[10px] font-semibold text-slate-600 hover:text-slate-900 hover:bg-slate-50 transition-colors">
              <FileText size={12} />
              GUIDE MÉTHODE
            </a>
          </div>
...
// Parser de fichier stock simple (référence + quantité de stock)
// Utilisé pour enrichir les rows obtenues depuis un fichier de ventes (NFP = 0 par défaut)
function parseStockFile(json) {
  const stockMap = {};
  let errorCount = 0;
  for (const raw of json) {
    const ref = String(findField(raw, DDMRP_ALIASES.ref) || '').trim();
    const stock = parseFloat(findField(raw, DDMRP_ALIASES.nfp));
    if (!ref || isNaN(stock)) { errorCount++; continue; }
    // Si plusieurs lignes pour la même référence (ex. multi-magasins), on agrège
    stockMap[ref] = (stockMap[ref] || 0) + stock;
  }
  return { stockMap, errorCount };
}

function mergeStockIntoRows(rows, stockMap) {
  let matched = 0;
  const newRows = rows.map(r => {
    if (stockMap[r.ref] != null) {
      matched++;
      return { ...r, nfp: stockMap[r.ref] };
    }
    return r;
  });
  return { newRows, matched, totalStockRefs: Object.keys(stockMap).length };
}

function generateStockTemplate() {
  return [
    { 'Référence': 'A001', 'Stock actuel': 1500 },
    { 'Référence': 'A002', 'Stock actuel': 800 },
    { 'Référence': 'A003', 'Stock actuel': 2400 },
  ];
}
...
function DdmrpBuffers({ onBack }) {
  ...
  // Import d'un second fichier de stock pour compléter les NFP des rows déjà chargées
  // (notamment après un import depuis l'historique de ventes, où NFP = 0 par défaut)
  const handleStockFileImport = async (e) => {
    const file = e.target.files?.[0];
    if (!file) return;
    try {
      const buf = await file.arrayBuffer();
      ...
      const { stockMap, errorCount } = parseStockFile(json);
      if (Object.keys(stockMap).length === 0) {
        throw new Error('Aucun stock valide trouvé. Le fichier doit contenir une colonne "Référence" et une colonne "Stock actuel".');
      }

      const { newRows, matched, totalStockRefs } = mergeStockIntoRows(rows, stockMap);
      setRows(newRows);

      const unmatched = totalStockRefs - matched;
      const parts = [`${matched}/${rows.length} article(s) renseigné(s)`];
      if (unmatched > 0) parts.push(`${unmatched} référence(s) du fichier non trouvée(s) dans la table`);
      if (errorCount > 0) parts.push(`${errorCount} ligne(s) ignorée(s)`);
      setImportMsg({ type: matched > 0 ? 'success' : 'warning', text: 'Stock importé : ' + parts.join(' · ') });
      setTimeout(() => setImportMsg(null), 7000);
    } catch (err) {
      setImportMsg({ type: 'error', text: 'Erreur d\'import stock : ' + err.message });
      setTimeout(() => setImportMsg(null), 7000);
    } finally {
  ...
  const downloadTemplate = (kind) => {
    if (kind === 'params') {
      ...
      const ws = XLSX.utils.json_to_sheet(generateStockTemplate());
      ws['!cols'] = [{ wch: 12 }, { wch: 16 }];
      const wb = XLSX.utils.book_new();
      XLSX.utils.book_append_sheet(wb, ws, 'Stock actuel');
      XLSX.writeFile(wb, 'atelier-template-stock.xlsx');
    } else {
      const ws = XLSX.utils.json_to_sheet(generateSalesTemplate());
      ws['!cols'] = [{ wch: 12 }, { wch: 24 }, { wch: 12 }, { wch: 16 }, { wch: 12 }, { wch: 12 }];
      const wb = XLSX.utils.book_new();
    ...
    <div className="min-h-screen text-slate-900" style={{ background: '#F8FAFC', fontFamily: "'DM Sans', system-ui, sans-serif" }}>
      ...
      {mode === 'batch' && (
        <div className="max-w-[1600px] mx-auto px-6 py-6 space-y-5">
          ...
          <div className="rounded-xl border border-slate-200 bg-white p-4 flex items-center justify-between flex-wrap gap-3">
            <div className="flex items-center gap-2 flex-wrap">
              ...
              {isSalesMode && (
                <>
                  <div className="w-px h-5 bg-slate-200 mx-1" />
                  <input ref={stockFileInputRef} type="file" accept=".xlsx,.xls,.csv" onChange={handleStockFileImport} className="hidden" />
                  <button onClick={() => stockFileInputRef.current?.click()} className="flex items-center gap-1.5 px-3 py-2 rounded-md font-jetbrains text-xs font-semibold text-white shadow-sm transition-all hover:translate-y-[-1px]" style={{ background: '#F59E0B' }}>
                    <Upload size={13} />
                    IMPORTER STOCK
                  </button>
                  <button onClick={() => downloadTemplate('stock')} className="flex items-center gap-1.5 px-3 py-2 rounded-md font-jetbrains text-xs text-slate-700 border border-slate-300 hover:bg-slate-50 transition-all">
                    <FileDown size={13} />
                    TEMPLATE STOCK
                  </button>
                </>
          ...
          {/* Bandeau d'avertissement : stocks tous à zéro après import historique */}
          {isSalesMode && rows.length > 0 && rows.every(r => !r.nfp || r.nfp <= 0) && (
            <div className="rounded-xl border-2 p-4 flex items-start gap-3" style={{ borderColor: '#F59E0B', background: '#FFFBEB' }}>
              <div className="flex-shrink-0 mt-0.5"><AlertTriangle size={20} style={{ color: '#B45309' }} /></div>
              <div className="flex-1">
                <div className="font-bricolage font-semibold text-slate-900 mb-1">Stocks actuels non renseignés</div>
                <div className="text-sm text-slate-700 leading-relaxed mb-3">
                  Votre fichier d'historique de ventes ne contenait pas l'état du stock. Les <strong>{rows.length} articles</strong> ont donc tous un stock à 0
                  et apparaissent en rupture (rouge) — ce qui n'est pas représentatif de votre situation réelle.
                  <br />
                  <strong className="text-slate-900">Importez votre fichier de stock actuel</strong> (référence + quantité) pour activer les statuts pertinents.
                </div>
                <button onClick={() => stockFileInputRef.current?.click()} className="inline-flex items-center gap-1.5 px-3 py-2 rounded-md font-jetbrains text-xs font-bold text-white shadow-sm transition-all hover:translate-y-[-1px]" style={{ background: '#F59E0B' }}>
                  <Upload size={13} />
                  IMPORTER LE STOCK MAINTENANT
                </button>
              </div>
            </div>
          )}
